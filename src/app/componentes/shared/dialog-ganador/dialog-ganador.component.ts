...
export class DialogGanadorComponent implements OnInit {
  ...
  response: Response;
  constructor(  private clipboard: Clipboard ,public dialog: MatDialog,public dialogRef: MatDialogRef<DialogGanadorComponent>, @Inject(MAT_DIALOG_DATA) public data: Persona, private serviceGanador:GanadorService, private serviceVigencia: VigenciaService) { 
  ...
  onCopyLink(sele:Ganador){
    ...
    this.clipboard.copy(sele.detallePremio.link);
    let id = sele.detallePremio.vigencia.id;
    this.actualizarVigencia(id);
  }
